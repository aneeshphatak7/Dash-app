...
dcc.Input(
    ...
    type='number',
    value=0.7,
    id = 'confidence',style={'width':'15%','display': 'inline-block'
            }
) ,
dcc.Input(
    ...
    type='number',
    value=0.4,
    id = 'support',style={'width':'15%','display': 'inline-block'
            }
) ,
...
dcc.Input(
    ...
    type='number',
    value=1,
    id = 'lift',style={'width':'15%','display': 'inline-block'
            }
) ,        
dcc.Dropdown(
    ...
    value='Attrition=No',
    id = 'Consequent', style={'width':'200px','display': 'inline-block'
            }
) ,        
...
dcc.Input(
    ...
    type='number',
    value=4,
    id = 'rules', style={'width':'15%','display': 'inline-block'
            }
) ,  
dcc.Dropdown(
    placeholder='sort by',
    ...
    value='lift',
    id = 'Sort_by', style={'width':'200px','display': 'inline-block'
            }
) , 
...
html.Div(id="table1"),]),
html.H3("Assosiation rules Plot ",style=dict(color='Blue',align='center',justify='center')),
        html.Div(id='graphs'),
...
def update_datatable(n_clicksx,confidence,support,lift,rules,Consequent,Sort_by):
    ...
    print("++++++++++++++++++++++++++++++++++++++")
    print(sup_rules.to_dict("rows"))
    tab=html.Div(children=[html.Br([]),html.Label(children='Top 5 rules', style={'width': '50%', 'display': 'inline-block',
                                                                 'margin': 0, 'padding': '8px','color':'blue'}),
...
def update_graph(nclick,confidence,support,lift,rules,Consequent,Sort_by):
    ...
    lif=rules['lift']
    #sup_rules = rules[['support','confidence','lift']]
    print(rules["support"])
...
#    plt.show()
    return html.Div([
            dcc.Graph(
        id='ScatterPlot Confidence Vs Support',
        figure={
                'data':[
                        go.Scatter(x=sup,
                         y= conf,
                ...
                'layout': go.Layout(
                        title ='Scatterplot of Confidence vs Support with lift as color gradient'
